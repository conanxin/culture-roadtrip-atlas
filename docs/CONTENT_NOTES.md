...
## OEDW 视觉资料细粒度扩展 v1.5.9 (OEDW Visual Bibliography Pack Granular) (2026-07-06)

在 v1.5.8 基础上扩展 3 个新模块，把视觉资料与参考书目准备扩展为细粒度版。

- 页面状态：完成度 93% · 视觉资料准备版
- 新增「22 条 Milestone 补图清单」（22 行 × 6 字段）
- 新增「OEDW 现场实拍计划」6 类（道路/人/地貌/边界/日常/资料照）
- 新增「视觉资料命名与归档规范」4 部分（目录/命名/分类/版权字段）
- 出发前核查清单从 5 类扩展为 8 类
...
- check-route-seo.py 接受 v1.5.2/3/4/5/6/7/8/9
- CSS 增强 130 行
...
- 页面状态：完成度 93% · 可信度与风险审计版（保持 93%，不强行升到 95% 或 100%）
- 明确说明：93% 指页面资料结构成熟度，不代表路线实地验证完成
- 新增「你应该从哪里开始」6 卡片
- 新增「本路线多维完成状态」9 维
- 新增「资料可信度说明」4 级（A/B/C/D）
- 新增「不建议普通旅行者逐点复刻的场景」8 类
- 新增「交通与可达性分级」10 段
- 新增「官方故事如何变成旅行观察任务」8 类
- 新增「FAQ」12 条
- 顶部导航简化为 8 个核心锚点
- 仍需验证清单从 8 项扩展为 13 项
- 页面版本号全量更新：v1.5.9 → v1.5.10
...
## OEDW 参考书目深度补全 v1.5.12 (OEDW Bibliography Deep Fill) (2026-07-07)

Out of Eden Walk 中国段新增参考书目地图、10 个主题资料分组、资料阅读优先级、参考资料使用方法和待补书目缺口；页面从结构化阅读版升级为参考资料增强版。

- 页面状态：完成度 93% · 参考资料增强版（保持 93%，不强行升到 95% 或 100%）
- 明确说明：93% 指页面资料结构成熟度，不代表实地验证完成
- 新增「OEDW 参考书目地图」10 主题 × 3 条资料（30 条资料条目）
- 每条资料标准化字段：类型 / 语言 / 可信度（A/B/C/D）/ 核验状态 / 链接
- 新增「参考资料阅读优先级」（必读 / 推荐 / 深读 / 待补）
- 新增「如何使用这些参考资料」（5 条使用指南）
- 新增「待补书目与资料缺口」（10 个主题各列出已有 / 缺口 / 下一步检索方向）
- 所有资料均通过官方网站 / UNESCO / 官方机构页面联网核实
- 不下载 / 不复制 / 不搬运任何官方或第三方图片
- 不强行提高完成度百分比
- 不改辽塔页面 / 不改山西页面 / 不改 CSV/GeoJSON/GPX/SVG 坐标
- 不新增地图 API / npm / 构建系统
...
## OEDW 页面质量收束 v1.5.13 (OEDW Page Health Check Stabilization) (2026-07-07)

Out of Eden Walk 中国段完成页面健康检查、折叠结构体检、quick nav 锚点检查、参考资料链接核查和 Milestone 官方链接复核；页面从参考资料增强版升级为稳定版候选。

- 页面状态：完成度 93% · 稳定版候选（保持 93%，不强行升到 95% 或 100%）
- 稳定版候选说明：页面结构、资料索引、路线执行、风险边界和数据资产已具备公开阅读与持续维护基础；不代表实地验证完成
- 新增「页面健康检查摘要」10 项核心模块状态卡（9 项 PASS · 1 项 PENDING = 实地验证）
- 新增 scripts/check-oedw-page-health.py 自动化页面体检脚本
- quick nav 10 个 pill 全部指向真实存在的 id
- 13 对 details 折叠结构全部闭合
- 12 张 table 全部在横向滚动容器内
- 30 条参考资料链接已核验
- 22 条 Milestone 官方 URL 已复核
- 旧状态残留清理：90% / 92% / 可信度与风险审计版（仅历史）/ 规划中（仅注释）
- 不下载 / 不复制 / 不搬运任何官方或第三方图片
- 不强行提高完成度百分比
- 不改辽塔页面 / 不改山西页面 / 不改 CSV/GeoJSON/GPX/SVG 坐标
- 不新增地图 API / npm / 构建系统
