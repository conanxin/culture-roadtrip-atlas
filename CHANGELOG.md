...
## v1.5.14 · OEDW 稳定版候选维护计划 (2026-07-07)

### 主要变更

- **OEDW 页面从「稳定版候选」进入「维护期」**
- **页面状态保持**：93% · 稳定版候选 / 维护期（不强行升到 95% 或 100%）
- **稳定版候选边界声明**：明确已稳定的 7 个领域（事实口径 / 页面结构 / 资料索引 / 风险边界 / 路线数据资产 / 自动化检查 / SEO OG）和仍不稳定的 7 个领域（实地照片 / 开放状态 / 交通现实 / 实走耗时 / 天气与季节风险 / 现场反馈 / 图片授权）

### 新增维护计划模块

- **新增「OEDW 稳定版候选维护计划」**：当前阶段说明 · 允许进入 patch 的改动（11 类） · 需要实地验证后才能改的内容（8 类） · 链接复查周期（5 类） · 图片与授权维护规则 · 版本升级规则
- **新增「OEDW 后续维护看板」**：12 项维护任务表格（维护项 / 当前状态 / 优先级 / 周期 / 触发条件 / 下一步），含 Milestone 官方链接复查 / 参考资料链接复查 / 图片授权状态 / 自有实拍图 / 点位开放状态 / 交通现实核查 / 参考书目深读 / Dispatch 二次深读 / 移动端页面体检 / SEO OG 微调 / 路线数据下载复查 / 用户反馈收集
- **新增「问题反馈与变更记录规则」**：5 类问题记录（事实错误 / 链接失效 / 旅行现实变化 / 图片与授权 / 体验问题）
- **新增「稳定版候选边界声明」**：两列对比（已稳定 vs 仍不稳定），附结论

### 脚本升级

- **check-oedw-page-health.py 升级**：新增 v1.5.14 维护期检查（OEDW 稳定版候选维护计划 / OEDW 后续维护看板 / 问题反馈与变更记录规则 / 稳定版候选边界声明 / 维护期 / manifest data_status_label 含维护期）
- **check-route-seo.py**：新增接受 v1.5.14

### 保持不变

- 路线数据不动：CSV / GeoJSON / GPX / SVG 坐标不修改
- 事实边界不动：22/22 · 6,000–6,700 · 北京段卢沟桥 → 天安门 → 小汤山 · 黄海终点 2023 冬 / 2024.6 / 2024.8 · 文化复刻粗点 · 非原始 GPS
- manifest SEO 字段保留 9/9
- manifest OEDW stats / 辽塔 / 山西字段未动
...
- **OEDW 页面从「参考资料增强版」升级为「稳定版候选」**
- **页面顶部状态保持**：93% · 稳定版候选（不强行升到 95% 或 100%）
- **明确说明**：93% 指页面资料结构成熟度，不代表实地验证完成
- **稳定版候选含义**：页面结构、资料索引、路线执行、风险边界和数据资产已具备公开阅读与持续维护基础

### 新增页面健康检查

- **新增「页面健康检查摘要」**（10 项核心模块状态卡）：
  - 内容结构 / Milestones 覆盖 / 折叠结构 / quick nav 锚点 / 长表格容器 / 事实边界 / 参考资料链接 / Milestone 官方 URL / 数据资产 / 实地验证
  - 9 项 PASS · 1 项 PENDING（实地验证）
- **新增 scripts/check-oedw-page-health.py**（自动化页面体检脚本）：
  - 基础内容 / 事实边界 / Milestone 74-95 覆盖 / 必需结构模块
  - details 折叠结构 / quick nav 锚点 / 长表格容器
  - manifest 一致性 / 旧状态残留（90% / 92% / 可信度与风险审计版 / 规划中）

### 质检与收束

- **quick nav 锚点检查**：10 个 pill 全部指向页面真实存在的 id（状态 / 阅读地图 / 官方故事 / 执行卡 / 参考书目 / 视觉资料 / 出发前 / 风险 / 数据 / FAQ）
- **折叠结构检查**：13 对 `<details>` 全部闭合，每个都含 `<summary>`
- **长表格容器检查**：12 张 `<table>` 全部在横向滚动容器内
- **参考资料链接核查**：30 条资料 27 条独立外链核验完成
- **Milestone 官方 URL 复核**：22 条 Milestone 官方 URL 全部来自 outofedenwalk.nationalgeographic.org
- **旧状态残留清理**：90% / 92% / 可信度与风险审计版（仅历史）/ 规划中（仅注释） 已确认
- **稳定版候选说明**：明示稳定版候选不等于实地验证完成
