...
Out of Eden Walk 中国段进入稳定版候选维护期。新增维护计划、patch 规则、链接复查周期、图片授权维护规则、后续维护看板、问题反馈与变更记录规则和稳定版候选边界声明。页面状态保持 93% · 稳定版候选 / 维护期。
...
- **v1.5.10 · OEDW 可信度、风险与读者入口审计**：页面从「视觉资料准备版」升级为「可信度与风险审计版」
- OEDW 页面状态保持 93% · 可信度与风险审计版（不强行升到 95% 或 100%）
- 新增「你应该从哪里开始」读者入口（6 卡片：了解项目/实际旅行/研究官方资料/拍摄写作/下载数据/判断可信度和风险）
- 新增「本路线多维完成状态」表（9 维度：事实审校/Milestones/Dispatch/行程执行卡/视觉资料/交通/开放状态/实地验证/参考书目）
- 新增「资料可信度说明」4 级（A 官方已复核 / B 可访问待深读 / C 中文解读 / D 待实地验证）
- 新增「不建议普通旅行者逐点复刻的场景」8 类（边境/深山/高海拔/保护区/野长城/港口/乡村私域/夜间跨城）
- 新增「交通与可达性分级」表（10 段路线 × 5 字段：普通公共交通/自驾包车/徒步/建议等级/说明）
- 新增「官方故事如何变成旅行观察任务」表（8 类官方主题 → 现场观察问题）
- 新增「FAQ」12 条（GPS/官方路线/逐点复刻/7天选段/28天适合/GPX导航/实地验证/100%状态/如何帮助/保留不确定性/阅读模式）
- 导航简化为 8 个核心锚点（总览/入口/状态/可信度/风险/可达性/FAQ/待验证）
- 顶部主状态从「93% · 视觉资料准备版」改为「93% · 可信度与风险审计版」
- 显式说明：93% 指页面资料结构成熟度，不代表路线实地验证完成
- 仍需验证清单从 8 项扩展为 13 项
- manifest v1.5.10：OEDW data_status_label = 长线文化复刻样板 · 可信度与风险审计版
- check-route-seo.py：接受 v1.5.2-1.5.10
- CSS 增强 200+ 行（.oedw-reader-entry / .oedw-status-table / .oedw-trust-levels / .oedw-trust-a/b/c/d / .oedw-risk-audit / .oedw-accessibility-table / .oedw-observation-table / .oedw-faq-list / .oedw-faq-item 等 22 个新类）
- 不下载 / 不复制 / 不搬运任何官方或第三方图片
- 不改辽塔页面 / 不改山西页面 / 不改 CSV/GeoJSON/GPX/SVG 坐标

v1.5.9 在 v1.5.8 基础上：

- **v1.5.9 · OEDW 视觉资料细粒度扩展**：在 v1.5.8 基础上补充 3 个新模块
- OEDW 页面状态 92% → 93% · 视觉资料准备版
- 新增「22 条 Milestone 补图清单」（22 行 × 6 字段：必补/可选/不建议/拍摄难度/备注）
- 新增「OEDW 现场实拍计划」（6 类：道路/人/地貌/边界/日常/资料照）
- 新增「视觉资料命名与归档规范」（目录/命名规则/分类/版权字段）
- 出发前核查清单从 5 类扩展为 8 类（路线与天气/开放状态/交通衔接/住宿区域/拍摄与记录/安全与伦理/资料复核/撤退策略）
- Milestone 视觉资料矩阵（重命名自「视觉资料占位与来源管理」）
- 参考书目与资料入口（重命名自「参考书目与延伸阅读」）
- 导航新增「补图清单/实拍计划/归档规范」3 个锚点
- 页面版本号全量更新：v1.5.8 → v1.5.9
- manifest v1.5.9：OEDW data_status_label = 长线文化复刻样板 · 视觉资料准备版
- check-route-seo.py：接受 v1.5.2-1.5.9
- CSS 增强 130+ 行
- 不下载 / 不复制 / 不搬运任何官方或第三方图片
- 不改辽塔页面 / 不改山西页面 / 不改 CSV/GeoJSON/GPX/SVG 坐标
